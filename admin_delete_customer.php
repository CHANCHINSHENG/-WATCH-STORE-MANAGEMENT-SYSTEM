...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT CartID FROM 12_cart WHERE CustomerID = ?");
        $stmt->execute([$customerId]);
        $cartIds = $stmt->fetchAll(PDO::FETCH_COLUMN);

        if (!empty($cartIds)) {
            $placeholders = implode(',', array_fill(0, count($cartIds), '?'));
            $stmt = $pdo->prepare("DELETE FROM 13_cart_item WHERE CartID IN ($placeholders)");
            $stmt->execute($cartIds);
        }

        $stmt = $pdo->prepare("DELETE FROM 12_cart WHERE CustomerID = ?");
        $stmt->execute([$customerId]);
